...
class WaitlistTable extends WpListTable {

	private static $where_validation_array = [
		'ID' => 	[ 'value' => 'p.ID=%d' ],
		'email' => 	[ 'value' => 'w.email=%s' ],
		'sent'  => 	[
	...
	private static $allowed_sort = ['post_title','email', 'client', 'added','sent'];
    /** Class constructor */
	...
	public static function get_rows( $per_page = 25, $page_number = 1 ) {
		...
		$sql = "SELECT w.id, w.product_id, w.email, w.added, w.sent, p.post_title, IF(w.user_id != 0, 1, 0) as client
	...
	public static function record_count() {
		...
		$sql =  "SELECT COUNT(w.id) AS elements_count
				FROM {$wpdb->prefix}calisia_waitlist_waitlist AS w 
				INNER JOIN {$wpdb->prefix}posts AS p ON w.product_id=p.ID";
	...
	public function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			case 'email':
				return '<a href="'.menu_page_url( 'waitlist', false ).'&email='.$item[ $column_name ].'">'.$item[ $column_name ].'</a>';
			case 'added':
			case 'sent':
	...
	function get_columns() {
		$columns = [
			...
			'email' => __( 'E-mail', 'calisia-waitlist' ),
			'client' => __( 'Customer', 'calisia-waitlist' ),
			'added'    => __( 'Added', 'calisia-waitlist' ),
			'sent'    => __( 'Sent', 'calisia-waitlist' ),
		];
